...
public static class TicketTriagePersistence
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ApplyPersistedResult(ticket, result, vocabulary, log);
        ...
        ILogger? logger = null)
    {
        ...
        var priorityNames = vocabulary.Priorities.Select(p => p.Name).ToList();
        var validatedPriority = MatchAllowed(result.SuggestedPriority, priorityNames, "suggestedPriority", log);
        ticket.AiTriageSuggestedPriority = validatedPriority;

        ticket.AiTriagePriorityReason = result.PriorityReason;

        var statusNames = vocabulary.Statuses.Select(s => s.Name).ToList();
